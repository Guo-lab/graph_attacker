...
######################################################################
####  Define dot and dropout for sparse tensors [used in GCN]  #######
...
#* ################################################################
class GCN:
    def __init__(self, sizes, An, X_obs, name="", with_relu=True, params_dict={'dropout': 0.5}, gpu_id=0, seed=-1):
        """
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope(name) as scope:
                ...
                print(type(X_obs[X_obs.nonzero()]))
                #//self.X_sparse = tf.SparseTensor(np.array(X_obs.nonzero()).T, X_obs[X_obs.nonzero()].A1, X_obs.shape)
                if(isinstance(X_obs[X_obs.nonzero()], scipy.sparse.lil.lil_matrix)):
                    self.X_sparse = tf.SparseTensor(np.array(X_obs.nonzero()).T, X_obs[X_obs.nonzero()].todense().A1, X_obs.shape)
                else:
                    self.X_sparse = tf.SparseTensor(np.array(X_obs.nonzero()).T, X_obs[X_obs.nonzero()].A1, X_obs.shape)
